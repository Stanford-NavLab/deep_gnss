...
# Define maximum value regression problem
class MaxItemsDataset(Dataset):
    def __init__(self):
        self.max_items = 10
        self.dim_feat = 5
        self.max_bag_size = 5
        ...
        self.value_items = torch.tensor(np.arange(self.max_items))/(self.max_items)
...
#             i_items = np.random.randint(self.max_bag_size-2) + 3
            ...
            self.labels[i] = torch.max(sub_vals)
